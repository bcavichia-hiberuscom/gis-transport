...
export function WeatherTab({ fleetVehicles, fleetJobs }: WeatherTabProps) {
  ...
  const renderWeatherCard = (routeInfo: any) => {
    const vehicle = fleetVehicles.find(
      (v) => String(v.id) === String(routeInfo.vehicle),
    );
    if (!vehicle) return null;

    return (
      <div
        key={routeInfo.vehicle}
        className="premium-card p-5 cursor-pointer group border-[#EAEAEA] hover:border-[#1C1C1C]/40 transition-all bg-white"
      >
          ...
          <div className="relative shrink-0">
            <div
              className={cn(
                ...
                routeInfo.riskLevel === "HIGH"
                  ? "bg-red-50 border-red-200 text-red-600"
                  : routeInfo.riskLevel === "MEDIUM"
            ...
            >
              {routeInfo.riskLevel === "HIGH" ? (
                ...
                <CloudRain strokeWidth={1.5} className="h-5 w-5" />
              ) : (
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                <h4 className="text-[13px] font-medium tracking-tight text-[#1C1C1C] truncate">
                  {vehicle.label}
                </h4>
            ...
            {routeInfo.alerts && routeInfo.alerts.length > 0 && (
              <div className="mt-4 space-y-2">
                {routeInfo.alerts.map((alert: any, idx: number) => (
                  <div
                    key={idx}
                    className="flex items-start gap-2.5 p-2.5 rounded bg-[#F7F8FA] border border-[#EAEAEA]"
                  >
                    <div className="mt-0.5 text-[#6B7280]">
                      {alert.event === "WIND" ? (
                        <Wind strokeWidth={1.5} className="h-3.5 w-3.5" />
                      ) : alert.event === "SNOW" ? (
                        <Snowflake strokeWidth={1.5} className="h-3.5 w-3.5" />
                      ) : (
                        <CloudRain strokeWidth={1.5} className="h-3.5 w-3.5" />
                      )}
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] font-bold text-[#1C1C1C] uppercase tracking-wider">
                        {alert.event}
                      </span>
                      <span className="text-[10px] text-[#6B7280] leading-snug pr-2">
                        {alert.message}
                      </span>
                    </div>
                  </div>
        ...
        <div className="px-10 py-10 flex flex-col sm:flex-row items-center justify-between gap-8">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-[13px] text-[#6B7280] font-normal mt-1">
              Análisis y mitigación de riesgos climáticos en las rutas
              operativas activas.
            </p>
          </div>
        </div>
                ...
                <div className="premium-card p-5 border-[#EAEAEA] bg-white group hover:border-[#D4F04A]/50 transition-all cursor-default">
                  ...
                  <div>
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      <span className="text-[10px] font-medium text-[#1C1C1C] uppercase tracking-wider">
                        Unidades
                      </span>
                    </div>
                  </div>
                  ...
                  <div>
                    <span
                      className={cn(
                        ...
                        kpis.generalRisk === "ELEVADO"
                          ? "text-red-600"
                          : kpis.generalRisk === "MODERADO"
                            ? "text-amber-600"
                            : "text-[#1C1C1C]",
                      )}
                    >
              ...
              <div className="bg-white border border-[#EAEAEA] rounded-lg p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                {isLoading ? (
                  <div className="py-20 col-span-full text-center bg-[#F7F8FA] rounded-md border border-dashed border-[#EAEAEA]">
                    <CloudDrizzle
                      strokeWidth={1.25}
                      className="h-8 w-8 text-[#6B7280]/30 mx-auto mb-3 animate-pulse"
                    />
                  ...
                  <div className="py-20 col-span-full text-center bg-[#F7F8FA] rounded-md border border-dashed border-[#EAEAEA]">
                    ...
                    />
                    <p className="text-[10px] font-medium uppercase tracking-wider text-[#6B7280]/50">
                      Sin impactos meteorológicos registrados
                    </p>
                  </div>
                )}
              </div>
