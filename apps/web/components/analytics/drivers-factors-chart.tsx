            ...
            <div className="h-[280px] w-full relative">
                {!hasData ? (
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-emerald-50/10 border border-dashed border-emerald-100 rounded-xl">
                        ...
                        <h4 className="text-[10px] font-black text-emerald-700 uppercase tracking-widest">Estado Nominal</h4>
                        <p className="text-[9px] font-bold text-emerald-600/50 uppercase mt-1">Sin factores de riesgo detectados</p>
                    </div>
                ) : (
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={data} layout="vertical" margin={{ top: 5, right: 30, left: 40, bottom: 5 }}>
                            <CartesianGrid strokeDasharray="4 4" horizontal={false} stroke="rgba(0,0,0,0.04)" />
                            ...
                            <YAxis
                                ...
                                tickLine={false}
                                width={120}
                                tick={{ fontSize: 8, fontWeight: 900, fill: "#64748b", width: 100 }}
                            />
                            <Tooltip
                            ...
                            <Bar
                                ...
                                label={{ position: 'right', fill: '#0f172a', fontSize: 10, fontWeight: 900, formatter: (val: number) => `${val} total` }}
                            >
                                {data.map((entry, index) => (
                                    <Cell key={`cell-${index}`} fill={entry.color || "#0f172a"} fillOpacity={1 - (index * 0.15)} />
                                ))}
                            </Bar>
                        </BarChart>
                    </ResponsiveContainer>
