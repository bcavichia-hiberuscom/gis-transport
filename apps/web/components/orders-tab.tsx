...
export function OrdersTab({
  ...
  const [activeTab, setActiveTab] = useState(
    fleetJobs.length === 0 ? "orders" : "overview",
  );
  const [currentPeriod, setCurrentPeriod] = useState<TimePeriod>("30d");
  const [visibleLogsJobId, setVisibleLogsJobId] = useState<
    string | number | null
  >(null);

  const analyticsData = useMemo(() => {
    const totalJobs = fleetJobs.length;
    const completedJobs = fleetJobs.filter(
      (j) => (j as any).status === "completed",
    ).length;
    const pendingJobs = fleetJobs.filter(
      (j) => (j as any).status === "pending",
    ).length;
    const inProgressJobs = fleetJobs.filter(
      (j) => (j as any).status === "in_progress",
    ).length;
    const completionRate =
      totalJobs > 0 ? Math.round((completedJobs / totalJobs) * 100) : 0;

    const totalDistance = fleetJobs.reduce(
      (acc, j) => acc + ((j as any).distance || 0),
      0,
    );
    ...
    const trendData = [
      {
        name: "Semana 1",
        completed: Math.max(0, completedJobs - 8),
        pending: Math.max(0, pendingJobs - 3),
        goal: 10,
      },
      {
        name: "Semana Actual",
    ...
    const leaderboardData = fleetJobs
      ...
      .sort(
        (a, b) =>
          (a.status === "critical" ? 1 : 0) -
            (b.status === "critical" ? 1 : 0) || b.value - a.value,
      )
      .slice(0, 5);
  ...
  const renderJobCard = (job: FleetJob) => {
    ...
    const assignedGroup = vehicleGroups.find(
      (g) => String(g.id) === String(job.assignedGroupId),
    );
    ...
    const distanceFormatted =
      distanceValue > 0 ? `${distanceValue.toFixed(1)} km` : null;
    ...
    return (
      ...
      >
        <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex flex-col min-w-0">
              <div className="flex items-center gap-2">
                ...
                {(job as any).priority === "high" && (
                  <span className="h-4 px-1.5 bg-red-50 text-red-700 text-[8px] font-medium rounded border border-red-100 flex items-center tracking-wider">
                    CRÍTICO
                  </span>
                )}
              </div>
          ...
          <ChevronRight
            strokeWidth={1.5}
            className="h-4 w-4 text-[#6B7280]/40 group-hover:text-[#1C1C1C] transition-all group-hover:translate-x-0.5"
          />
        </div>

        <div className="grid grid-cols-2 gap-4 pt-4 border-t border-[#F7F8FA]">
          <div className="flex flex-col gap-1">
            <span className="text-[9px] font-medium text-[#6B7280] uppercase tracking-wider">
              Asignación
            </span>
            {assignedVehicle ? (
              <div className="flex items-center gap-2">
                <div className="h-1.5 w-1.5 rounded-full bg-[#D4F04A]" />
                <span className="text-[11px] font-medium text-[#1C1C1C] truncate">
              ...
              <span className="text-[10px] font-normal text-[#6B7280]/40 uppercase">
                Pendiente
              </span>
            )}
          </div>
        ...
        {distanceFormatted && (
          <div className="pt-3 border-t border-[#F7F8FA] flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Navigation
                strokeWidth={1.5}
                className="h-3 w-3 text-[#6B7280]/40"
              />
            ...
            {(job as any).status === "completed" && (
              <div className="flex items-center gap-1.5 bg-[#D4F04A]/10 px-2 py-0.5 rounded border border-[#D4F04A]/20">
                ...
                <span className="text-[9px] font-medium text-[#5D6B1A] tracking-tight">
                  SLA OK
                </span>
              </div>
            )}
        ...
        <div className="px-10 py-10 flex items-center justify-between gap-10">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-[13px] text-[#6B7280] font-normal mt-1">
              Planificación operativa y optimización dinámica de la cadena de
              suministro.
            </p>
          </div>
              ...
              >
                <tab.icon
                  ...
                  className={cn(
                    ...
                    isActive ? "text-[#1C1C1C] scale-110" : "text-[#6B7280]/60",
                  )}
                />
                {tab.label}
                {isActive && (
                ...
                <div className="flex flex-col gap-10 relative z-10 p-4">
                  ...
                  >
                    <div className="relative mb-8">
                      <Package
                        strokeWidth={1.25}
                        className="h-16 w-16 text-[#6B7280]/20 group-hover:scale-105 transition-all group-hover:text-[#1C1C1C]"
                      />
                    ...
                    <p className="text-[11px] font-normal text-[#6B7280] text-center max-w-sm leading-relaxed">
                      No se han detectado órdenes en el sistema. Inicie una
                      nueva operación para activar los algoritmos de
                      optimización.
                    </p>
                  </div>
                </div>
                  ...
                  <div className="flex flex-col h-full bg-white border border-[#EAEAEA] rounded-md shadow-sm overflow-hidden group hover:border-[#D0D0D0] transition-colors">
                    <div className="shrink-0 flex items-center justify-between px-6 py-4 border-b border-[#EAEAEA] bg-white relative">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-[11px] font-bold uppercase tracking-widest text-[#1C1C1C]">
                          Pedidos Pendientes
                        </h3>
                      </div>
                    ...
                    <div className="flex-1 p-4 bg-[#F7F8FA]/50 flex flex-col gap-4 overflow-y-auto">
                      {fleetJobs.filter(
                        ...
                        <div className="py-20 text-center bg-white rounded-md border border-dashed border-[#EAEAEA] flex flex-col items-center justify-center h-full">
                          ...
                          />
                          <p className="text-[10px] font-medium uppercase tracking-wider text-[#6B7280]/50">
                            Cola Vacía
                          </p>
                        </div>
                      )}
                      ...
                      <Badge className="bg-[#1C1C1C] text-[#D4F04A] h-6 px-3 border border-[#1C1C1C] font-bold tabular-nums rounded text-[10px]">
                        {
                          fleetJobs.filter(
                            (j) =>
                              (!!j.assignedVehicleId || !!j.assignedGroupId) &&
                              (j as any).status !== "completed",
                          ).length
                        }
                      </Badge>
                  ...
                  <div className="flex flex-col h-full bg-white border border-[#EAEAEA] rounded-md shadow-sm overflow-hidden group hover:border-[#D0D0D0] transition-colors">
                    ...
                    <div className="flex-1 p-4 bg-[#F7F8FA]/50 flex flex-col gap-4 overflow-y-auto">
                      {fleetJobs.filter(
                        ...
                        fleetJobs
                          .filter((j) => (j as any).status === "completed")
                          .map(renderJobCard)
                      ) : (
                        <div className="py-20 text-center bg-white rounded-md border border-dashed border-[#EAEAEA] flex flex-col items-center justify-center h-full">
                          <Zap
