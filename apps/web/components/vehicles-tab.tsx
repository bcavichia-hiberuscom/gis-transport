...
export function VehiclesTab({
  ...
  const analyticsData = useMemo(() => {
    const maintenanceCount = fleetVehicles.filter(
      (v) => (v as any).status === "maintenance",
    ).length;
    const availableCount = fleetVehicles.length - maintenanceCount;
    const totalConsumption = fleetVehicles.reduce(
      (acc, v) => acc + ((v as any).fuelConsumption || 0),
      0,
    );
    const avgConsumption =
      fleetVehicles.length > 0 ? totalConsumption / fleetVehicles.length : 0;
    const utilizationRate =
      fleetVehicles.length > 0
        ? Math.round((availableCount / fleetVehicles.length) * 100)
        : 0;
    ...
    return {
      ...
      leaderboardData: fleetVehicles.map((v, idx) => ({
        ...
        status: (((v as any).fuelConsumption || 10) > 15
          ? "critical"
          : "good") as "critical" | "good" | "warning",
      })),
      trends: {
        ...
        <div className="relative shrink-0">
          ...
          <span
            className={cn(
              ...
              (vehicle as any).status !== "maintenance"
                ? "bg-[#D4F04A]"
                : "bg-[#6B7280]",
            )}
          />
          ...
          <div className="flex items-start justify-between">
            <div className="min-w-0 flex-1">
              <h4 className="text-[13px] font-medium tracking-tight text-[#1C1C1C] truncate leading-tight">
                {(vehicle as FleetVehicle).licensePlate ||
                  vehicle.label ||
                  vehicle.id}
              </h4>
              <p className="text-[10px] font-medium text-[#6B7280] mt-0.5 uppercase tracking-wide">
                {(vehicle as FleetVehicle).brand || "GLOBAL"} •{" "}
                {(vehicle as FleetVehicle).type.label}
              </p>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <span
                className={cn(
                  ...
                  (vehicle as FleetVehicle).status !== "maintenance"
                    ? "bg-[#D4F04A]/10 text-[#5D6B1A] border-[#D4F04A]/20"
                    : "bg-[#F7F8FA] text-[#6B7280] border-[#EAEAEA]",
                )}
              >
                {(vehicle as FleetVehicle).status !== "maintenance"
                  ? "OPERATIVO"
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[10px] font-medium text-[#1C1C1C] tabular-nums">
                  {((vehicle as FleetVehicle).mileage || 0).toLocaleString()}{" "}
                  <span className="text-[8px] ml-0.5">km</span>
                </span>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[10px] font-medium text-[#1C1C1C] tabular-nums">
                  {((vehicle as FleetVehicle).fuelConsumption || 0).toFixed(1)}{" "}
                  <span className="text-[8px] ml-0.5">L/100</span>
                </span>
              </div>
            </div>
          </div>
                ...
                <div className="flex flex-col h-full">
                  <div className="shrink-0 flex items-center justify-between px-6 py-4 bg-[#F7F8FA] border border-[#EAEAEA] rounded-t-lg border-b-none">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-[11px] font-medium uppercase tracking-wider text-[#1C1C1C]">
                  ...
                  <div className="bg-white border-x border-[#EAEAEA] border-b rounded-b-lg p-6 flex flex-col gap-4">
                    {groups.available.length > 0 ? (
                      ...
                      <div className="py-20 text-center bg-[#F7F8FA] rounded-md border border-dashed border-[#EAEAEA]">
                        <Truck
                          strokeWidth={1.25}
                          className="h-8 w-8 text-[#6B7280]/30 mx-auto mb-3"
                        />
          ...
          {activeTab === "maintenance" && (
            <div className="animate-in fade-in duration-200 p-10">
              <div className="premium-card p-16 text-center border-[#EAEAEA] bg-[#F7F8FA] rounded-lg">
                <Wrench
                  strokeWidth={1}
                  className="h-12 w-12 text-[#6B7280]/20 mx-auto mb-6"
                />
                <h3 className="text-sm font-medium uppercase tracking-wider text-[#1C1C1C] mb-2">
                  Mantenimiento Predictivo
                </h3>
                <p className="text-[#6B7280] text-[12px] font-normal max-w-xs mx-auto leading-relaxed">
                  Sincronización de telemetría y análisis de ciclos de vida en
                  proceso.
                </p>
              </div>
            </div>
