...
import { Search, UserCheck, Users, ChevronRight, X, Filter, Target } from "lucide-react";
                    ...
                    <div className="flex items-center gap-4 mb-8">
                        ...
                        <div>
                            ...
                            <DialogDescription className="text-[10px] uppercase tracking-widest text-[#6B7280]/60 mt-0.5">
                                {vehicleLabel ? `Destino: ${vehicleLabel}` : "Selección de Personal Activo"}
                            </DialogDescription>
                        </div>
                    </div>
                                ...
                                {filteredDrivers.length > 0 ? (
                                    filteredDrivers.map((driver) => (
                                        <div
                                            ...
                                            className="w-full flex items-center justify-between p-4 rounded border border-[#EAEAEA] hover:border-[#1C1C1C]/40 bg-white transition-all cursor-pointer group"
                                        >
                                            <div className="flex items-center gap-4">
                                                <div className="h-10 w-10 rounded bg-[#F7F8FA] border border-[#EAEAEA] flex items-center justify-center shrink-0">
                                                    {driver.imageUrl ? (
                                                        ...
                                                        <Users strokeWidth={1.5} className="h-5 w-5 text-[#6B7280]/30" />
                                                    )}
                                                </div>
                                            ...
                                            <ChevronRight strokeWidth={1.5} className="h-4 w-4 text-[#6B7280]/20 group-hover:text-[#1C1C1C] group-hover:translate-x-0.5 transition-all" />
                                        </div>
                                    ))
                                ) : (
                                    <div className="py-16 text-center bg-[#F7F8FA] rounded border border-dashed border-[#EAEAEA]">
                                        <Users strokeWidth={1} className="h-10 w-10 text-[#6B7280]/20 mx-auto mb-3" />
